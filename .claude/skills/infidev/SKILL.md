...
1. **Never hold full file contents in context.** Read only the specific lines you need, then act immediately.
2. **Never recap or summarize work to the user.** They can see the diffs. Just do the work.
3. **Keep all output terse.** One line per action. No explanations unless something failed.
4. **Use subagents liberally** to offload research, exploration, and complex tasks — their context doesn't count against yours.

## The loop

Each iteration:

1. **Decide what to do** — Pick ONE high-impact task. Prioritize in this order:
